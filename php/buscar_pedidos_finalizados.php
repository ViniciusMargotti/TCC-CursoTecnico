...
$sql = "SELECT * FROM pedidos where pedidos.status= 'Finalizado' and id_pedido LIKE '%".strtoupper($_POST['like'])."%' order by TIME(hora_pedido) desc  ";
...
while($registro = mysqli_fetch_array($resultado_id, MYSQLI_ASSOC)){
	...
	echo "<tr>";
	echo "<td width='2%' style=' font-size:17px; font-weight:600;'  text-transform: uppercase;'  value='" . $registro['id_pedido'] . "' >#" . $registro['id_pedido']  . "</td>"; 
	...
	echo"<td   >";
	echo "<button style='width:100%;'  onclick='AlteracaoPedido(". $registro['id_pedido']." , 3) '; class='btn btn-success'>  <i class='material-icons'>done</i></button>";
	echo"</td>";
